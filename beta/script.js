...
const capitalEvolutionChartCanvas = document.getElementById('capitalEvolutionChart').getContext('2d');
const salesByModelChartCanvas = document.getElementById('salesByModelChart').getContext('2d');
const expenseEvolutionChartCanvas = document.getElementById('expenseEvolutionChart').getContext('2d');
...
let currentCapitalEvolutionChart = null;
let currentSalesByModelChart = null;
let currentExpenseEvolutionChart = null;
...
function updateDashboardCharts(boutique, startDate = null, endDate = null) {
    ...
    updateCapitalEvolutionChart(boutique, startDate, endDate);
    updateSalesByModelChart(boutique, startDate, endDate);
    updateExpenseEvolutionChart(boutique, startDate, endDate);
}
...
function updateCapitalEvolutionChart(boutique, startDate = null, endDate = null) {
    const stockRef = boutique === 'Toutes' ? database.ref('stock') : database.ref(`stock/${boutique}`);
    stockRef.once('value', snapshot => {
        const stockData = snapshot.val();
        ...
        if (stockData) {
            for (const produit in stockData) {
                const details = stockData[produit];
                const monthlyCapitalValue = details.prixAchat * (details.stockInitial !== undefined ? details.stockInitial : 0);
                const monthYear = `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}`; // Assuming current month for simplicity; adjust as needed for historical data
                if (!capitalByMonth[monthYear]) {
                    capitalByMonth[monthYear] = 0;
                }
                capitalByMonth[monthYear] += monthlyCapitalValue;
            }

            const sortedMonths = Object.keys(capitalByMonth).sort();
            const labels = sortedMonths.map(monthYear => monthYear);
            const data = sortedMonths.map(monthYear => capitalByMonth[monthYear]);

            if (currentCapitalEvolutionChart) {
                currentCapitalEvolutionChart.destroy();
            }

            currentCapitalEvolutionChart = new Chart(capitalEvolutionChartCanvas, {
                type: 'line',
                data: {
                    labels: labels,
                    datasets: [{
                        label: 'Évolution du Capital Mensuel',
                        data: data,
                        borderColor: 'rgba(54, 162, 235, 1)',
                        fill: false
                    }]
...
function updateSalesByModelChart(boutique, startDate = null, endDate = null) {
    const ventesRef = boutique === 'Toutes' ? database.ref('ventes') : database.ref(`ventes/${boutique}`);
    ventesRef.once('value', snapshot => {
        const salesData = snapshot.val();
        const salesByModel = {};
            ...
            for (const boutiqueKey in salesData) {
                if (boutique === 'Toutes' || boutiqueKey === boutique) {
                    for (const saleId in salesData[boutiqueKey]) {
                        const sale = salesData[boutiqueKey][saleId];
                         if ((!startDate || sale.date >= startDate) && (!endDate || sale.date <= endDate)) { // Date filter
                            if (!salesByModel[sale.produit]) {
                                salesByModel[sale.produit] = 0;
                            }
                            salesByModel[sale.produit] += sale.prixTotal;
                         }
                    }
                }
            }

            const models = Object.keys(salesByModel);
            const data = Object.values(salesByModel);
            const backgroundColors = models.map((_, index) => `rgba(${index * 50 % 255}, ${index * 70 % 255}, ${index * 90 % 255}, 0.8)`);
            const borderColors = models.map((_, index) => `rgba(${index * 50 % 255}, ${index * 70 % 255}, ${index * 90 % 255}, 1)`);


            if (currentSalesByModelChart) {
                currentSalesByModelChart.destroy();
            }

            currentSalesByModelChart = new Chart(salesByModelChartCanvas, {
                type: 'bar',
                data: {
                    labels: models,
                    datasets: [{
                        label: 'Ventes par Modèle de Téléphone',
                        data: data,
                        backgroundColor: backgroundColors,
                        borderColor: borderColors,
                        borderWidth: 1
...
function updateExpenseEvolutionChart(boutique, startDate = null, endDate = null) {
    const depensesRef = boutique === 'Toutes' ? database.ref('depenses') : database.ref(`depenses/${boutique}`);
    depensesRef.once('value', snapshot => {
        const expensesData = snapshot.val();
        const expensesByMonth = {};
        if (expensesData) {
            for (const boutiqueKey in expensesData) {
                 if (boutique === 'Toutes' || boutiqueKey === boutique) {
                    for (const expenseId in expensesData[boutiqueKey]) {
                        const expense = expensesData[boutiqueKey][expenseId];
                        if ((!startDate || expense.dateDepense >= startDate) && (!endDate || expense.dateDepense <= endDate)) {
                            ...
                            expensesByMonth[monthYear] += parseFloat(expense.montantDepense);
                        }
                    }
                 }
            }
            ...
            const labels = sortedMonths.map(monthYear => monthYear);
            const data = sortedMonths.map(monthYear => expensesByMonth[monthYear]);

            if (currentExpenseEvolutionChart) {
                currentExpenseEvolutionChart.destroy();
            }

            currentExpenseEvolutionChart = new Chart(expenseEvolutionChartCanvas, {
                type: 'line',
                data: {
                    labels: labels,
                    datasets: [{
                        label: 'Évolution des Dépenses Mensuelles',
                        data: data,
                        borderColor: 'rgba(255, 99, 132, 1)',
                        fill: false
                    }]
